...
export default function FlashMessage({ message }: FlashMessageProps) {
    ...
    return createPortal(
        <div className="font-display fixed top-20 left-1/2 z-300 w-full max-w-70 -translate-x-1/2 rounded-md border-2 border-white/20 bg-card-backdrop-gray px-2 font-medium text-white backdrop-blur-sm sm:max-w-110">
            <button
            ...
            <div className="flex items-center justify-center p-2">
                <p className="flex items-center gap-2 text-center text-balance">
                    <CheckIcon className="size-7 shrink-0 text-white" />
                    {message}
                </p>
            </div>
        </div>,
