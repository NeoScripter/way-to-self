    ...
    return (
        ...
        >
            <div
                aria-hidden="true"
                className={cn(
                    'absolute top-0 bottom-0 left-1/2 -z-10 w-screen -translate-x-1/2',
                    className,
                )}
            ></div>
...
function NavMenu({ className, close, show }: NavMenuProps) {
    ...
    return (
        <div
            className={cn(
                'fixed top-0 right-0 left-0 z-10 h-200 max-h-screen overflow-y-auto bg-light-swamp bg-cover !bg-bottom-left transition-transform duration-500 ease-in-out md:static md:flex md:h-auto md:flex-1 md:items-center md:justify-between md:gap-5 md:bg-transparent md:!bg-none',
                className,
        ...
        >
            ...
            <PrimaryBtn
                className="mx-auto shrink-0 text-xl md:order-2 md:mr-0 md:text-xs xl:text-base"
                href="/"
            >
                Личный кабинет
            </PrimaryBtn>
...
function NavLink({ children }: NavLinkProps) {
    return (
        <li className="transition-colors duration-200 ease-in hover:text-very-bright-salad">
            {children}
        </li>
    );
}
